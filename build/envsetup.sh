...
cat <<EOF
Additional Pearl functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- pearlgerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
...
function pearlremote()
{
    ...
    local PEARL_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$PEARL_USER" ]
    then
        git remote add pearl ssh://review.pearl.org:29418/$PFX$PROJECT
    else
        git remote add pearl ssh://$pearl_USER@review.pearl.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/PEARL/$PROJECT
...
function pearlgerrit() {
    ...
    local user=`git config --get review.review.pearlos.org.username`
...
function pearlrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      pearlrebase <path to project> <patch IDs on Gerrit>"
